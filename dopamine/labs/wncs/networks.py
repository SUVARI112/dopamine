...
from dopamine.labs.wncs import atari_lib
...
def preprocess_wncs_inputs(x):
  """Input Flattening for Wireless Network Control System input."""
...
### DQN Networks ###
@gin.configurable
class NatureDQNNetworkWNCS(nn.Module):
  """Modified DQN Network for vector-based inputs."""
  """
  When __init__ is called on this class, it will set up the initial configuration of the network with
  two main parameters:
  num_actions: Required parameter that defines the number of possible actions (output dimensions)
  inputs_preprocessed: Optional boolean parameter that defaults to False, indicating whether inputs need preprocessing
  The actual layer initialization doesn't happen during __init__. This is a Flax neural network using the 
  nn.Module system, which uses @nn.compact for lazy initialization. The actual layer initialization occurs
    when the network is first called with input data.
  """
  num_actions: int
  inputs_preprocessed: bool = False
  ...
  def __call__(self, x):
    ...
    # if not self.inputs_preprocessed:
    #   x = preprocess_wncs_inputs(x)

    # Convert input to float32 and flatten if needed
    x = x.astype(jnp.float32)
    x = x.reshape((-1))  # flatten vector input
...
### Rainbow Networks ###
@gin.configurable
class RainbowNetworkWCNS(nn.Module):
...
class ImplicitQuantileNetworkWNCS(nn.Module):
...
class PPOSharedNetworkWCNS(nn.Module):
    ...
    def __call__(self, x) -> jnp.ndarray:
        ...
        # Dynamically build the network based on num_layers and hidden_units
        for _ in range(self.num_layers):
            x = nn.Dense(features=self.hidden_units, kernel_init=initializer)(x)
            x = getattr(nn, self.activation)(x) if isinstance(self.activation, str) else self.activation(x)
        return x
...
class PPODiscreteActorCriticNetworkWCNS(nn.Module):
    ...
    def setup(self):
        action_dim = functools.reduce(operator.mul, self.action_shape, 1)
        self._shared_network = PPOSharedNetworkWCNS(
