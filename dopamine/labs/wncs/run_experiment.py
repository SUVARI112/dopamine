...
from dopamine.labs.wncs.env import create_wncs_environment
...
def create_runner(base_dir, schedule='continuous_train_and_eval'):
  ...
  # Only runs evaluation using existing checkpoints
  elif schedule == 'eval':
...
class Runner(object):
  ...
  def __init__(
      ...
      base_dir,
      create_agent_fn,
      create_environment_fn=create_wncs_environment,
...
class EvalRunner(Runner):
    """Object that handles running evaluation-only experiments."""
    ...
    def __init__(
        ...
        """Initialize the EvalRunner object."""
        ...
        latest_checkpoint = checkpointer.get_latest_checkpoint_number(self._checkpoint_dir)
        if latest_checkpoint < 0:
            raise ValueError(
                f'No checkpoints found in {self._checkpoint_dir}. Cannot run evaluation.'
            )
        
        # Load the latest checkpoint
        experiment_data = self._checkpointer.load_checkpoint(latest_checkpoint)
        if not self._agent.unbundle(
            self._checkpoint_dir, latest_checkpoint, experiment_data
        ):
    ...
    def _run_eval_phase(self, statistics):
        ...
        while step_count < self._evaluation_steps:
            # Handle both cases: with and without max_steps_per_episode
            if self._max_steps_per_episode is None:
                episode_length, episode_return = self._run_continued_episode(
                    step_count, self._evaluation_steps
                )
            else:
                episode_length, episode_return = self._run_one_episode()
    ...
    def run_experiment(self):
        """Runs evaluation-only experiment."""
        logging.info('Beginning evaluation...')
        
        # Run specified number of evaluation iterations
        for iteration in range(self._num_iterations):
            ...
            if self._has_collector_dispatcher:
                self._collector_dispatcher.write([
                    statistics_instance.StatisticsInstance(
                        'Eval/NumEpisodes', num_episodes, iteration
                    ),
                    statistics_instance.StatisticsInstance(
                        'Eval/AverageReturns', average_reward, iteration
                    ),
                ])
            ...
            if self._summary_writer is not None:
                self._save_tensorboard_summaries(
                    iteration, num_episodes, average_reward
                )
    ...
    def _save_tensorboard_summaries(self, iteration, num_episodes, average_reward):
        """Save statistics as tensorboard summaries."""
        if self._summary_writer is None:
            return
        ...
        if self._sess is None:
            with self._summary_writer.as_default():
                ...
                tf.summary.scalar('Eval/AverageReturns', average_reward, step=iteration)
            self._summary_writer.flush()
        else:
            summary = tf.compat.v1.Summary(
                value=[
                    tf.compat.v1.Summary.Value(
                        tag='Eval/NumEpisodes', simple_value=num_episodes
                    ),
                    tf.compat.v1.Summary.Value(
                        tag='Eval/AverageReturns', simple_value=average_reward
                    ),
                ]
            )
            self._summary_writer.add_summary(summary, iteration)
